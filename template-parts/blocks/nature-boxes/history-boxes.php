...
//print_r($all_stories);
$featured_stories = array_slice($all_stories, 0, 2);
$remaining_stories = array_slice($all_stories, 2);
?>
...
<section class="historier-oversigten container mx-auto px-4 mt-10">
  ...
  <?php if (!empty($remaining_stories)) : ?>
    <div class="historier-groen-grid mt-10">
      <?php foreach ($remaining_stories as $story) : ?>
        ...
        $title = get_the_title($story->ID);
        ?>
        <a href="<?php echo esc_url($permalink); ?>" class="historier-groen-kort">
          <?php if ($tema) : ?>
