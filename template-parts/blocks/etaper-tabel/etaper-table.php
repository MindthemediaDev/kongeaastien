<?php
if (is_admin()) {
  echo '<p>Etape listing vises kun på frontend.</p>';
  return;
}
...
$headline = "Se Kongeåstiens etaper og rundture her"; //get_field('headline');
            ...
            <div class="p-6">
              <?php if ($rundtur || $rundtur_tekst || $rundture) : ?>
                <div class="space-y-4">
                                    ...
                                    <span style="display:none;" class="inline-flex rounded-full bg-lightgreen px-4 py-1 text-sm text-white">
                                        Rundtur
                                    </span>
                  ...
                  <?php if (!empty($rundture) && is_array($rundture)) : ?>
                    <div class="space-y-3 leading-snug">
                      <?php foreach ($rundture as $item) : ?>
                        ...
                        <?php if ($titel || $km) : ?>
                          <div>
                            <?php $link = get_permalink($rundtur_post_id); ?>

                            <a href="<?php echo esc_url($link); ?>" class="inline-flex items-center gap-2">

                              <?php get_template_part('template-parts/icons/icon-rundtur'); ?>

                              <span>
                <?php echo esc_html($titel); ?>
                                <?php if ($km) : ?>
                                  <span class="ml-1"><?php echo esc_html($km); ?> km.</span>
                                <?php endif; ?>
            </span>
